...
const withFileContext = (fn) => async (req, res, next) =>
  fn({ req, res, next })
    .then((filename) => {
...
const withJsonContext = (fn) => async (req, res, next) =>
  fn({ req, res, next })
    .then((result) => {
      res.json(result);
    })
...
export const httpErrorFromMollyError = (mollyError) => {
  ...
  if (isNotFoundError(mollyError)) {
    return new HttpError(mollyError.message, 404);
  }
  console.error(mollyError);
...
export const getObject = (path, readObjectFn) => (router) =>
  router.get(
    path,
    withFileContext(J.composeP(readObjectFn, withParams(J.identity))),
  );
